...
/* 使用static变量ClientState来链接客户端状态（即Clients类型的m->c） */
		...
		int persistence_type, void* persistence_context)
{
	...
	/* ????handle是每个客户端mqtt_new创建时malloc(sizeof(mqtt_client))的1个成员MQTTClient，在这里用于记录实体对象malloc(sizeof(MQTTClients))的地址 */
...
thread_return_type WINAPI MQTTClient_run(void* n)	//-*使用一个专门的线程维护一个客户端的运行。。。。可以维护多个建立过链接的客户端。。
{
...
//-下面针对校验过的参数进行连接，对链接各个步骤需要等待的或者失败的进行处理
...
void MQTTClient_yield(void)	//-客户端放弃,里面有周期处理函数,可以实现发送和接收还有处理
{
	...
	if (running)
	{
		/* MQTTClient_run正在运行，（MQTTClient_connectURIVersion启动的） */
