...
{
	int rc = 0;
	MQTTClient_persistence* per = NULL;	//-一个结构体用于存储持久存储的信息

	FUNC_ENTRY;
#if !defined(NO_PERSISTENCE)
	switch (type)	//-即使定义了持久存储,在创建客户端的时候还可以选择持久存储的类型
	{//-存储其实就是保存在非易失性空间内,这里可以根据配置选择存储的地方
		case MQTTCLIENT_PERSISTENCE_NONE :
			per = NULL;
			break;
		case MQTTCLIENT_PERSISTENCE_DEFAULT :		//-让系统分配
			per = malloc(sizeof(MQTTClient_persistence));
			if ( per != NULL )
			{
				if ( pcontext != NULL )
				{
					per->context = malloc(strlen(pcontext) + 1);	//-描述表,可能是用于说明整个持久存储空间的;或者是说明存储路径的
					strcpy(per->context, pcontext);
				}
				else
					per->context = ".";  /* working directory */
				/* file system functions */
				per->popen        = pstopen;	//-开辟了一个空间记录持久存储信息,这里都是对这些信息进行填写的
				per->pclose       = pstclose;
				per->pput         = pstput;
				per->pget         = pstget;
			...
			break;
		case MQTTCLIENT_PERSISTENCE_USER :	//-用户指定了存储的指针
...
int MQTTPersistence_persistQueueEntry(Clients* aclient, MQTTPersistence_qEntry* qe)	//-呵呵,又是一个入口,还是申请空间吧,只是这个空间赋予的的含义不同,通过结构体定义区分
{
	...
	if ((rc = aclient->persistence->pput(aclient->phandle, key, nbufs, (char**)bufs, lens)) != 0)	//-向持久路径写东西
